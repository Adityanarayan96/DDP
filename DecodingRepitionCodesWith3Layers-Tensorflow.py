...
input_vector_length = output_vector_length*repitions
hidden_layer_nodes = output_vector_length*repitions
learning_rate = 1e-3
...
B1 = tf.Variable(tf.random_normal([1,hidden_layer_nodes],stddev=0.3,dtype = "float64"),name ="Input_Layer_biases", dtype = "float64")
B2 = tf.Variable(tf.random_normal([1,output_vector_length],stddev=0.3,dtype = "float64"),name ="Input_Layer_biases", dtype = "float64")
W1 = tf.Variable(tf.random_normal([input_vector_length,hidden_layer_nodes],stddev=0.3,dtype = "float64"),name = "Input_Layer", dtype ="float64")
W2 = tf.Variable(tf.random_normal([hidden_layer_nodes,output_vector_length],stddev=0.3,dtype = "float64"),name = "Input_Layer", dtype ="float64")
...
Hidden_Layer = tf.sigmoid(tf.add(tf.matmul(Input_codes,W1),B1))
...
for i in range(0,len(MessageWordsArray)):
    coded_train[i,:] = RepetitionCodes(MessageWordsArray[i,:],repitions) + AddNoise(0,input_vector_length)
coded_validation = np.zeros([len(MessageWordsArray),input_vector_length])
for i in range(0,len(MessageWordsArray)):
    coded_validation[i,:] = RepetitionCodes(MessageWordsArray[i,:],repitions) + AddNoise(0,input_vector_length)
...
with tf.Session() as sess: #Start the session
	...
	for epoch in range(epochs):
		...
		for i in range(total_batch):
			...
			_,c,w1,w2,b1,b2,ber_final,output_layer = sess.run([optimizer,mse,W1,W2,B1,B2,BER,Output_layer],feed_dict = {Input_codes: coded_train, Output_desired_codes: MessageWordsArray})
